...
| 截图翻译 | 已实现基础版 | `captureRegionAndTranslate()` / `translateLastScreenshot()` / `HotKeyAction.translateScreenshot` | 支持从菜单或默认 `⌘⌥T` 直接框选区域截图并翻译，也可翻译最近截图；OCR 后优先使用 Google 在线端点，失败后自动切换 MyMemory 备用端点，仍失败时打开网页翻译兜底；应用内展示原文/译文并复制译文 |
...
3. 截图翻译已有框选翻译、应用内结果窗口、主备在线端点和网页兜底，尚未接入离线翻译引擎。
