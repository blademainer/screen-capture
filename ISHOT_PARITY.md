...
| 录屏 | 已实现 | `startRecording()` / `HotKeyAction.startRecording` / `HotKeyAction.togglePauseRecording` / `CaptureStreamOutput.setPaused()` | 支持全屏、窗口和拖拽选择区域录制，并按目标显示器、目标窗口或选区尺寸配置编码输出；默认 `⌥W` 开始录屏，`⌘⌥Space` 暂停/恢复并在输出层跳过暂停样本、压缩时间戳，`⌘⇧R` 停止录屏 |
...
| 录音 | 已实现 | `startAudioRecording()` / `HotKeyAction.startAudioRecording` / `HotKeyAction.togglePauseRecording` / `CaptureStreamOutput(audioOnly:)` | 支持独立 `.m4a` 录音，可录制系统内部声音和麦克风，默认 `⌘⇧M` 开始录音，`⌘⌥Space` 暂停/恢复并复用输出层暂停处理、按请求来源验收音频帧/音轨 |
