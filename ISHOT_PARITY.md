...
| 长截图 | 已实现基础版 | `captureScrollingWindow()` | 自动滚动、检测相邻截图重叠区域并裁剪后纵向拼接；后续可升级为窗口边界识别 |
| 多窗口截图 | 已实现 | `captureMultipleWindowsScreenshot()` | 调用 macOS 交互窗口选择，按住 Shift 连续选择窗口；默认启用桌面底板模式，可在设置中关闭 |
| 全屏带壳截图 | 已实现 | `captureDeviceFramedFullScreen()` | 生成 Mac 风格设备外壳，无水印 |
| 圆角与阴影 | 已实现 | `applyOutputStyle()` | 可配置圆角半径、阴影大小和阴影颜色 |
| 标注 | 已实现 | `EditingTool` / `ImageEditingSession` / `FloatingWindowContentView` | 画笔、荧光笔、矩形、圆形、箭头、文字、数字序号、马赛克、裁剪；数字序号支持配置起始值并连续编号 |
...
| 截图翻译 | 已实现基础版 | `translateLastScreenshot()` | OCR 后在线翻译，应用内展示原文/译文并复制译文，保留网页翻译兜底 |
...
1. 长截图已做相邻截图重叠检测和重复区域裁剪，尚未自动识别滚动容器边界。
2. 多窗口截图已支持桌面底板模式，当前仍依赖 macOS 系统交互能力，尚未实现完全自绘的多窗口选区界面。
3. 截图翻译已有应用内结果窗口，当前默认使用在线翻译端点，尚未接入离线翻译引擎。
4. 数字序号标注已支持起始编号偏好；后续可继续增加单个序号的拖动重排和样式模板。
