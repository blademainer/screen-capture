...
- **音频录制**: 同时录制系统音频和麦克风（v2.0 重新实现）
  - 智能权限管理和设备检测
  - 高质量音频（48kHz, 192kbps AAC）
  - 独立的音频轨道
  - 详细的日志和错误处理
...
如果遇到麦克风录制问题，可以使用内置的调试工具：

```bash
# 1. 运行应用并进行录制测试

# 2. 检查调试日志
./scripts/check_microphone_log.sh

# 3. 查看详细指南
cat MICROPHONE_DEBUG_GUIDE.md
```

**日志文件位置**: `~/Desktop/MacScreenCapture_Microphone_Debug.log`

详细使用说明请参考 [麦克风调试指南](MICROPHONE_DEBUG_GUIDE.md)
