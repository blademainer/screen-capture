...
| 截图翻译 | 已实现基础版 | `captureRegionAndTranslate()` / `translateLastScreenshot()` | 支持从菜单直接框选区域截图并翻译，也可翻译最近截图；OCR 后优先使用 Google 在线端点，失败后自动切换 MyMemory 备用端点，仍失败时打开网页翻译兜底；应用内展示原文/译文并复制译文 |
