...
| 多窗口截图 | 已实现 | `captureMultipleWindowsScreenshot()` / `MultiWindowSelectionView` / `HotKeyAction.multiWindowScreenshot` | 自绘窗口多选覆盖层；窗口截图时按住 Shift 可连续选择多个窗口并按 Enter 合成，独立多窗口模式默认 `⌘⌥W` 触发，桌面底板模式可在设置中关闭 |
...
2. 多窗口截图已支持自绘多选、窗口截图 Shift 连选和桌面底板模式；后续可继续补多显示器跨屏合成的完整验证。
