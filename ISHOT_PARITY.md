...
| 长截图 | 已实现 | `captureScrollingWindow()` | 自动滚动、按鼠标所在窗口裁剪切片、默认最多 30 屏且可配置到 100 屏，滚动到底自动停止、检测相邻截图重叠区域并裁剪后纵向拼接 |
...
1. 长截图已支持按鼠标所在窗口裁剪、滚动到底自动停止、最多 100 屏配置并检测重叠区域；后续可继续升级为网页/列表内部滚动容器的精确边界识别。
