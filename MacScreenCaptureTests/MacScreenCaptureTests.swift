...
final class MacScreenCaptureTests: XCTestCase {
    ...
    func testAnnotationSettingsExposeNumberStyleAndTemplateControls() throws {
        ...
        XCTAssertTrue(settingsSource.contains("presentSettingsPanel(panel) { response in"))
        XCTAssertTrue(settingsSource.contains("guard response == .OK, let url = panel.url else { return }"))
    ...
    func testBackgroundResponsivenessDisablesAppNapForHotKeys() throws {
        ...
        let hotKeySource = try repositoryFileContents("MacScreenCapture/Core/HotKeyManager.swift")
        let latencySource = try repositoryFileContents("MacScreenCapture/Core/HotKeyLatencyDiagnostics.swift")
        let captureManagerSource = try repositoryFileContents("MacScreenCapture/Core/CaptureManager.swift")
        let infoPlist = try repositoryFileContents("MacScreenCapture/Info.plist")

        XCTAssertTrue(appSource.contains("Self.setupRuntimeServices(captureManager: sharedCaptureManager)"))
        XCTAssertTrue(appSource.contains("private static var didSetupRuntimeServices = false"))
        XCTAssertTrue(appSource.contains("guard !didSetupRuntimeServices else { return }"))
        XCTAssertTrue(appSource.contains("_ = HotKeyManager.shared"))
        ...
        XCTAssertTrue(appSource.contains("private func refreshApplicationState()"))
        XCTAssertTrue(appSource.contains("permissionManager.checkAllPermissions()"))
        XCTAssertTrue(appSource.contains("captureManager.initialize()"))
        XCTAssertTrue(responsivenessSource.contains("ProcessInfo.processInfo"))
        XCTAssertTrue(responsivenessSource.contains(".userInitiatedAllowingIdleSystemSleep"))
        XCTAssertTrue(responsivenessSource.contains(".latencyCritical"))
        ...
        XCTAssertTrue(responsivenessSource.contains("func performHotKeyActivity<T>"))
        ...
        XCTAssertTrue(responsivenessSource.contains("private var readinessHeartbeatTimer: DispatchSourceTimer?"))
        XCTAssertTrue(responsivenessSource.contains("private var lastReadinessHeartbeatUptimeNanoseconds"))
        XCTAssertTrue(responsivenessSource.contains("private let heartbeatQueue"))
        XCTAssertTrue(responsivenessSource.contains("private let stateLock = NSLock()"))
        XCTAssertTrue(responsivenessSource.contains("startReadinessHeartbeat()"))
        XCTAssertTrue(responsivenessSource.contains("stopReadinessHeartbeat()"))
        XCTAssertTrue(responsivenessSource.contains("func hotKeyWakeMetadata() -> [String: String]"))
        XCTAssertTrue(responsivenessSource.contains("heartbeat_gap_ms"))
        XCTAssertTrue(hotKeySource.contains("Task(priority: .userInitiated) { @MainActor in"))
        XCTAssertTrue(hotKeySource.contains("BackgroundResponsivenessManager.shared.performHotKeyActivity"))
        XCTAssertTrue(hotKeySource.contains("@MainActor\n    private func performAction(_ action: HotKeyAction) async"))
        XCTAssertFalse(hotKeySource.contains("DispatchQueue.main.async {\n                    self.performAction(action)"))
        XCTAssertTrue(latencySource.contains("@TaskLocal static var current: HotKeyLatencyTrace?"))
        XCTAssertTrue(latencySource.contains("category: \"HotKeyLatency\""))
        XCTAssertTrue(latencySource.contains("elapsed_ms="))
        XCTAssertTrue(latencySource.contains("hotkey-latency.log"))
        XCTAssertTrue(latencySource.contains("appendPersistentRecord"))
        XCTAssertTrue(latencySource.contains("DispatchQueue(label: \"com.blademainer.MacScreenCapture.hotkey-latency-diagnostics\""))
        XCTAssertTrue(latencySource.contains("metadata: [String: String] = [:]"))
        XCTAssertTrue(latencySource.contains("metadataSuffix"))
        XCTAssertTrue(hotKeySource.contains("HotKeyLatencyDiagnostics.makeTrace(action: action.rawValue)"))
        XCTAssertTrue(hotKeySource.contains("HotKeyLatencyDiagnostics.$current.withValue(trace)"))
        XCTAssertTrue(hotKeySource.contains("HotKeyLatencyDiagnostics.mark(\"hotkey_received\", metadata: BackgroundResponsivenessManager.shared.hotKeyWakeMetadata())"))
        XCTAssertTrue(hotKeySource.contains("HotKeyLatencyDiagnostics.mark(\"task_started\")"))
        XCTAssertTrue(captureManagerSource.contains("HotKeyLatencyDiagnostics.mark(\"region_screenshot_started\")"))
        XCTAssertTrue(captureManagerSource.contains("HotKeyLatencyDiagnostics.mark(\"region_overlay_preparing\")"))
        XCTAssertTrue(captureManagerSource.contains("HotKeyLatencyDiagnostics.mark(\"region_overlay_ordered\")"))
    ...
    func testSettingsPanelButtonsAreBackedByVisibleActions() throws {
        let settingsSource = try repositoryFileContents("MacScreenCapture/Views/SettingsView.swift")
        let appSource = try repositoryFileContents("MacScreenCapture/MacScreenCaptureApp.swift")
        let contentSource = try repositoryFileContents("MacScreenCapture/ContentView.swift")
        let infoPlist = try repositoryFileContents("MacScreenCapture/Info.plist")
        let entitlements = try repositoryFileContents("MacScreenCapture/MacScreenCapture.entitlements")
        let captureManagerSource = try repositoryFileContents("MacScreenCapture/Core/CaptureManager.swift")
        let windowAccessorSource = try repositoryFileContents("MacScreenCapture/Utils/WindowAccessor.swift")
        let windowManagerSource = try repositoryFileContents("MacScreenCapture/Core/WindowManager.swift")

        XCTAssertTrue(settingsSource.contains("SettingsActionButton(title: defaultSaveLocation.isEmpty ? \"选择文件夹\""))
        ...
        XCTAssertTrue(settingsSource.contains("private struct SettingsActionButton: NSViewRepresentable"))
        XCTAssertTrue(settingsSource.contains("NSButton(title: title, target: context.coordinator, action: #selector(Coordinator.performAction))"))
        XCTAssertTrue(settingsSource.contains("@State private var settingsWindow: NSWindow?"))
        XCTAssertTrue(settingsSource.contains("WindowAccessor { window in"))
        XCTAssertTrue(settingsSource.contains("presentSettingsPanel(panel) { response in"))
        XCTAssertTrue(settingsSource.contains("persistDefaultSaveLocation(url)"))
        XCTAssertTrue(settingsSource.contains("url.bookmarkData(options: [], includingResourceValuesForKeys: nil, relativeTo: nil)"))
        XCTAssertFalse(settingsSource.contains("url.bookmarkData(options: .withSecurityScope"))
        XCTAssertFalse(settingsSource.contains("url.startAccessingSecurityScopedResource()"))
        XCTAssertTrue(captureManagerSource.contains("let resolutionOptions: [URL.BookmarkResolutionOptions] = [.withSecurityScope, []]"))
        XCTAssertTrue(captureManagerSource.contains("resolveDefaultSaveLocationBookmark(startAccessingSecurityScope: true)"))
        ...
        XCTAssertTrue(settingsSource.contains("SettingsActionButton(title: openAfterCaptureAppPath.isEmpty ? \"选择 App\""))
        XCTAssertTrue(settingsSource.contains("guard response == .OK, let url = panel.url else { return }"))
        XCTAssertTrue(settingsSource.contains("NSApp.activate(ignoringOtherApps: true)"))
        XCTAssertTrue(settingsSource.contains("window.makeKeyAndOrderFront(nil)"))
        XCTAssertTrue(settingsSource.contains("panel.beginSheetModal(for: window, completionHandler: completion)"))
        XCTAssertTrue(settingsSource.contains("panel.level = .modalPanel"))
        XCTAssertFalse(settingsSource.contains("panel.level = .floating"))
        XCTAssertFalse(settingsSource.contains("panel.orderFrontRegardless()"))
        XCTAssertTrue(windowAccessorSource.contains("DispatchQueue.main.async"))
        XCTAssertTrue(windowAccessorSource.contains("coordinator.lastWindow !== window"))
        XCTAssertFalse(windowAccessorSource.contains("if let window = nsView.window {\n            onWindowChange(window)"))
        XCTAssertTrue(appSource.contains("WindowGroup(\"Mac Screen Capture\")"))
        XCTAssertFalse(appSource.contains(".windowStyle(.hiddenTitleBar)"))
        XCTAssertFalse(contentSource.contains("HeaderView()"))
        XCTAssertFalse(contentSource.contains("xmark.circle.fill"))
        XCTAssertTrue(infoPlist.contains("<string>MacScreenCapture</string>"))
        XCTAssertTrue(infoPlist.contains("<string>com.blademainer.MacScreenCapture</string>"))
        XCTAssertTrue(infoPlist.contains("<string>13.0</string>"))
        XCTAssertTrue(infoPlist.contains("<key>LSUIElement</key>\n\t<false/>"))
        XCTAssertFalse(infoPlist.contains("$(DEVELOPMENT_LANGUAGE)"))
        XCTAssertFalse(infoPlist.contains("$(MACOSX_DEPLOYMENT_TARGET)"))
        XCTAssertFalse(infoPlist.contains("NSMainStoryboardFile"))
        XCTAssertFalse(entitlements.contains("com.apple.security.app-sandbox"))
        XCTAssertFalse(entitlements.contains("com.apple.security.files.user-selected.read-write"))
    ...
    func testRegionScreenshotUsesMagneticWindowSelection() throws {
        ...
        XCTAssertTrue(source.contains("magneticRegionForCurrentContext(candidates: magneticCandidates)"))
        XCTAssertTrue(source.contains("frontmostWindowRegion(candidates: candidates)"))
        XCTAssertTrue(source.contains("magneticRegionUnderMouse(candidates: candidates)"))
        ...
        XCTAssertTrue(source.contains("let capturePoint = quartzPoint(fromViewPoint: point)"))
        XCTAssertTrue(source.contains("magneticCandidates.first(where: { $0.bounds.contains(capturePoint) })"))
        XCTAssertTrue(source.contains("private final class RegionSelectionWindow: NSWindow"))
        XCTAssertTrue(source.contains("override var canBecomeKey: Bool { true }"))
        XCTAssertTrue(source.contains("override var canBecomeMain: Bool { true }"))
        XCTAssertTrue(source.contains("let selectionFrames = coordinateSpaces"))
        XCTAssertTrue(source.contains("for selectionFrame in selectionFrames"))
        XCTAssertTrue(source.contains("RegionSelectionWindow("))
        XCTAssertTrue(source.contains("selectionWindow.acceptsMouseMovedEvents = true"))
        XCTAssertTrue(source.contains("activeRegionSelectionWindows = selectionWindows"))
        ...
        XCTAssertTrue(source.contains("updateMagneticSelection(atScreenPoint: NSEvent.mouseLocation)"))
        XCTAssertFalse(source.contains("kAXFocusedWindowAttribute"))
        XCTAssertFalse(source.contains("kAXMainWindowAttribute"))
        XCTAssertTrue(source.contains("DisplayCoordinateSpace"))
        XCTAssertTrue(source.contains("activeDisplayCoordinateSpaces()"))
        XCTAssertTrue(source.contains("NSScreenNumber"))
        XCTAssertTrue(source.contains("coordinateSpaces: coordinateSpaces"))
        XCTAssertTrue(source.contains("coordinateSpace(containingScreenPoint:"))
        XCTAssertTrue(source.contains("coordinateSpace(forCaptureRect:"))
    ...
    func testRegionScreenshotShowsSelectionBeforeShareableContentRefresh() throws {
        ...
        guard let captureStart = source.range(of: "func captureScreenshot(autoOpenAfterCapture: Bool = true, forceSave: Bool = false)")?.lowerBound,
              let captureStateUpdate = source.range(
                of: "WindowManager.shared.updateCaptureState(.screenshotting)",
        ...
        XCTAssertLessThan(regionFastPath.lowerBound, captureStateUpdate.lowerBound)
        ...
        XCTAssertTrue(regionFunction.contains("WindowManager.shared.hideMainWindowForCaptureIfNeeded()"))
        XCTAssertFalse(regionFunction.contains("mainWindowVisible"))
        XCTAssertFalse(regionFunction.contains("Task.sleep(nanoseconds: 120_000_000)"))
        XCTAssertFalse(regionFunction.contains("Task.sleep(nanoseconds: 160_000_000)"))

        let windowManagerSource = try repositoryFileContents("MacScreenCapture/Core/WindowManager.swift")
        XCTAssertTrue(windowManagerSource.contains("func hideMainWindowForCaptureIfNeeded() -> Bool"))
    ...
    func testInlineCanvasAcceptsFirstDrawingClickAfterToolbarSelection() throws {
        ...
        let floatingPanelSetting = try XCTUnwrap(controllerSource.range(of: "window.isFloatingPanel = true"))
        let finalLevelSetting = try XCTUnwrap(controllerSource.range(of: "window.level = level", range: floatingPanelSetting.upperBound..<controllerSource.endIndex))
        XCTAssertLessThan(floatingPanelSetting.lowerBound, finalLevelSetting.lowerBound)
        XCTAssertTrue(controllerSource.contains("logInlineToolbarLayout"))
        XCTAssertTrue(canvasSource.contains("logInlineCanvasMouseDown"))
