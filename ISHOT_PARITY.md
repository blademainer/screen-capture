...
| 录屏 | 已实现 | `startRecording()` / `HotKeyAction.startRecording` | 支持全屏、窗口和拖拽选择区域录制；默认 `⌥W` 开始录屏，`⌘⇧R` 停止录屏 |
| 系统音频 | 已实现 | `SCStreamConfiguration.capturesAudio` / `RecordingAudioDiagnostics` | iShot Pro 高级项；停止录制后会等待文件写入完成并验收音频帧/音轨，异常时通知用户检查 |
| 录音 | 已实现 | `startAudioRecording()` / `HotKeyAction.startAudioRecording` / `CaptureStreamOutput(audioOnly:)` | 支持独立 `.m4a` 录音，可录制系统内部声音和麦克风，默认 `⌘⇧M` 开始录音，并复用音频帧/音轨验收 |
| 麦克风录音 | 已实现 | `SCStreamConfiguration.captureMicrophone` | 权限和设备检查已接入 |
| 录制前参数 | 已实现 | `startRecordingWithPreflight()` / `RecordingPreflightSettingsView` / `RecordingSettingsView` / `CaptureStreamOutput` | 菜单、热键和主录制按钮都会先确认清晰度、FPS、开录延时、系统音频、麦克风、光标、MOV/MP4；质量档位会实际影响 H.264 码率/编码配置 |
