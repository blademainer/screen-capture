...
struct RecordingAudioDiagnostics: Sendable {
    ...
    var hasAudioIssue: Bool {
        ...
        if audioTrackCount < requestedAudioSourceCount { return true }
...
class CaptureManager: ObservableObject {
        ...
        do {
            ...
            for index in 0..<sliceCount {
                ...
                if stopWhenUnchanged,
                   ...
                   ScrollingImageStitcher.imagesAreVisuallySimilar(previous, sliceImage) {
                    break
                }
    ...
    private func applyOutputStyle(to image: NSImage) -> NSImage {
        let style = ScreenshotStyleRenderer.OutputStyle(
            roundedCorners: UserDefaults.standard.bool(forKey: "screenshotRoundedCorners"),
            cornerRadius: CGFloat(UserDefaults.standard.double(forKey: "screenshotCornerRadius")),
            dropShadow: UserDefaults.standard.bool(forKey: "screenshotDropShadow"),
            shadowRadius: CGFloat(UserDefaults.standard.double(forKey: "screenshotShadowRadius")),
            shadowColor: ColorCodeFormatter.colorFromHex(UserDefaults.standard.string(forKey: "screenshotShadowColorHex") ?? "#000000") ?? .black
        )
        return ScreenshotStyleRenderer.applyOutputStyle(to: image, style: style)
    }
    ...
    private func renderDeviceFrame(around image: NSImage) -> NSImage {
        ...
        let style = ScreenshotStyleRenderer.DeviceFrameStyle(
            bezel: bezel,
            padding: padding,
        ...
        return ScreenshotStyleRenderer.renderDeviceFrame(around: image, style: style)
    }
    ...
    private func recognizeText(in image: NSImage) async throws -> String {
        ...
        return try await withCheckedThrowingContinuation { continuation in
            let request = VNRecognizeTextRequest { request, error in
                ...
                let textBoxes = observations.compactMap { observation -> OCRTextOrderer.TextBox? in
                    guard let text = observation.topCandidates(1).first?.string else { return nil }
                    return OCRTextOrderer.TextBox(text: text, boundingBox: observation.boundingBox)
                }
                let text = OCRTextOrderer.joinedText(textBoxes)
